pipeline {
    ...
    stages {        
      stage('SCM') {
         steps {
            git 'https://github.com/poornima608/example-voting-app.git'
         }
      }
      stage('Build Project') {
         steps {
    ...
    stage('Docker Image'){
        steps{
            ...
            echo "Build Images"
            cd vote
            sudo docker build -t 521997/vote-app .
            cd ../result
            sudo docker build -t 521997/result-app .
            cd ../worker
            sudo docker build -t 521997/worker-app .
             '''
           }
    }
    stage('Push Images to repository'){
        steps{
            ...
            echo "Push to repository"
             docker push 521997/vote-app
             docker push 521997/result-app
             docker push 521997/worker-app
             usermod -a -G docker ubuntu
             usermod -a -G 521997 root
            '''
            }
        }
    }
